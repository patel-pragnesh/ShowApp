...
var idPresentacionLocal = args.idPresentacionLocal;
var aDataDocuments = new DataBaseQuery().getDocumentsForNewPresentation(idPresentacionLocal);
...
for (var i = 0; i < iTotalFiles; i++) {
  ...
  botonForOpenFile[i] = Ti.UI.createButton({
    ...
    idPresentationOnline:aDataDocuments[i].id_presentation_online,
  });
  botonForOpenFile[i].addEventListener("click",function(e){
    var fileName = e.source.filePath;
    var mime = e.source.mimeType;
    var idPresentationOnline = e.source.idPresentationOnline;
    openFile(fileName,mime,idPresentationOnline);
  });
...
var tmpFile = undefined,
    newPath = undefined;
function openFile(fileName,mimeType,idPresentation){
